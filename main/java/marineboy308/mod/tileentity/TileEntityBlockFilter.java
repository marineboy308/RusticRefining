package marineboy308.mod.tileentity;

import java.util.Random;

import marineboy308.mod.container.ContainerBlockFilter;
import marineboy308.mod.objects.blocks.BlockFilter;
...
public class TileEntityBlockFilter extends TileEntityLockable implements ITickable, ISidedInventory {
    ...
    public void setInventorySlotContents(int index, ItemStack stack)
    {
        ...
        if (index == 0 && !flag)
        {
            this.totalFilterTime = this.getFilterTime(stack);
            this.filterTime = 0;
            this.markDirty();
        }
    }
    ...
    {
        return this.hasCustomName() ? this.filterCustomName : "container.block_filter";
    }
        ...
        if (!this.world.isRemote) {
            ...
            if (this.isFiltering() || !itemstack.isEmpty()) {
                if (!this.isFiltering() && this.canFilter()) {
                    this.filteringTime = this.getFilterTime(this.filterItemStacks.get(0));
                ...
                if (this.isFiltering() && this.canFilter())
                {
                    ...
                    {
                        this.filterTime = 0;
                        this.totalFilterTime = this.getFilterTime(this.filterItemStacks.get(0));
                        this.filterItem();
                        flag1 = true;
                    }
                }
    ...
    public int getFilterTime(ItemStack stack)
    {
        return 200;
    }
    ...
    public void filterItem() {
        if (this.canFilter()) {
            ...
            if (output1.isEmpty()) {
                this.filterItemStacks.set(1, result1.copy());
            } else if (output1.getItem() == result1.getItem()) {
    ...
    public boolean isItemFilterable(ItemStack stack) {
    	ItemStack result = FilterRecipes.instance().getFilteringResult(this.filterItemStacks.get(0));
    	if (!result.isEmpty()) {
    		return true;
    	}
    ...
    {
        return "rusticrefining:block_filter";
    }
